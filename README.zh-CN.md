...
约好同一个房间号，大家用同一条命令就进同一个聊天室。

```
$ clichat 404
* 已连接到房间「404」，代号 emoo
[14:23] <幽灵> 还有谁也 404 进来了？
[14:23] <emoo> 我，找不到 /home
> _
```
...
复制 `export ...` 那行扔到任何人面前就行。`Ctrl+C` 关闭。
...
进房间后输入 `/help` 可以随时打开完整命令清单。

| 命令 / 按键 | 效果 |
| --- | --- |
| 输入文字回车 | 发送消息 |
| `/help` `/?` | 列出全部命令 |
| `/r` `/reply` | 方向键选要引用的消息，Return 确认 |
| `/cowsay <文字>` | 把一段话用 ASCII 牛包起来发出去 |
| `/new` | 当场起一个本地服务并跳到一个随机房间 |
| `/clear` `/refresh` `/404` `Ctrl+L` | 清屏 |
| `Ctrl+C` | 退出 |

### 内嵌小游戏

两个游戏都跑在加密信道上，纯 ASCII 渲染，不依赖任何外部依赖。
...
| 命令 | 效果 |
| --- | --- |
| `/goldminer` `/gm` | 发起一局，地图按种子全员同步 |
| `/join` | 加入矿场（pending 状态） |
| `/start` | 发起者开始游戏 |
| `/g` `/aim` | 进入瞄准：钟摆中 Return 放钩，Esc 结束本回合 |

每人轮流操控钩子 30 秒，钩到的金块/钻石/石头按表得分；所有人各打完一轮后自动出排行榜。在 aim 模式里随便按字母会自动退出回到聊天，30 秒回合不会被打断。
...
**开服那台 Mac 老是自动休眠？**
另起一个终端跑 `caffeinate -i &`，要下线时 `kill %1` 关掉。
